...
The [CD workflow](../.github/workflows/deploy.yml) rsyncs `services/policy-search/`
to the server and restarts the systemd service whenever those files change on `main`
(or when run manually from the Actions tab). It logs in over SSH with a dedicated key.
...
## 3. Deploy user + directories (run on the server, as root/sudo)
...
sudo mkdir -p /opt/integrations/services/policy-search
...
echo 'deploy ALL=(root) NOPASSWD: /usr/bin/systemctl restart policy-search' \
...
```sh
sudo pip install -r /opt/integrations/services/policy-search/requirements.txt
...
```sh
sudo -u deploy tee /opt/integrations/services/policy-search/.env >/dev/null <<'EOF'
FLASK_ENV=production
FLASK_DEBUG=False
DATA_FILE=policies.json
EOF
sudo chmod 640 /opt/integrations/services/policy-search/.env
```

`.env` is excluded from rsync, so deploys never overwrite or delete it.

## 7. Install and start the service

```sh
sudo cp /opt/integrations/services/policy-search/../../deploy/policy-search.service \
        /etc/systemd/system/policy-search.service
# (or copy deploy/policy-search.service from a checkout of the repo)
sudo systemctl daemon-reload
sudo systemctl enable --now policy-search
systemctl status policy-search
```

## 8. Verify

```sh
curl http://158.39.201.47/health      # {"status":"ok","data_loaded":true}
journalctl -u policy-search -f        # live logs
```

After this, pushing a change to `services/policy-search/` on `main` (or running the
workflow manually) will sync the files and restart the service automatically.
